...
if PY2:
    ...
    def unic(item):
        ...
        if isinstance(item, unicode):
            print >>f1,"Found Unicode, returning item"
            return item
        if isinstance(item, (bytes, bytearray)):
            print >>f1,"Found bytearray"
            try:
                print >>f1, "Returning ASCII by decode"
                return item.decode('ASCII')
            except UnicodeError:
                print >>f1, "Returning custom u Code"
                return u''.join(chr(b) if b < 128 else '\\x%x' % b
                                for b in bytearray(item))
        try:
            try:
                print >>f1, "Returning unicode of Item"
                return unicode(item)
            except UnicodeError:
                print >>f1, "Returning Unic of Str of Item"
